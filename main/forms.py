...
class ProfileEditForm(forms.Form):
    ...
    def clean_first_name(self):
        ...
        if len(first_name) < 3:
            raise forms.ValidationError('Слишком короткое Имя - не меньше 3 символов')
        return first_name

    def clean_last_name(self):
        last_name = self.cleaned_data['last_name']
        if len(last_name) < 3:
            raise forms.ValidationError('Слишком короткая Фамилия - не меньше 3 символов')
        return last_name

    def clean_city(self):
        city = self.cleaned_data['city']
        if len(city) < 3:
            raise forms.ValidationError('Слишком короткое название города - не меньше 3 символов')
        return city

    def clean_age(self):
        age = self.cleaned_data['age']
        if age > 150:
            raise forms.ValidationError('Ошибка - не больше 150')
        return age
